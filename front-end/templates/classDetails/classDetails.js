async function showClassDetails(classId, className) {
    ...
    if (role === 'teacher') {
        ...
        addForm.addEventListener('submit', async e => {
            ...
            try {
                const res = await fetch('http://localhost:13371/classroom/add_question', {
                    method: 'POST',
                    headers: {
...
async function loadDashboard(classId) {
    ...
    const resp = await fetch("http://localhost:13371/classroom/dashboard", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
...
async function loadStudentList(classId) {
    const token = localStorage.getItem("token");
    const resp = await fetch("http://localhost:13371/classroom/students", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
