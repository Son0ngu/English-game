document.addEventListener('DOMContentLoaded', function () {
    ...
    // JWT Token management - use the token from login
    ...
    async function createGameRoom(difficulty = "easy", classId = "class1") {
        try {
            ...
            const response = await fetch('http://127.0.0.1:13371/game/newroom', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                if (response.status === 401 || response.status === 500) {
                    ...
                    console.log('Token expired or unauthorized, removing tokens and redirecting');
    ...
    async function getQuestion() {
        ...
        try {
            ...
            const response = await fetch('http://127.0.0.1:13371/game/get_question', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem("token")}`
            ...
            if (!response.ok) {
                if (response.status === 401 || response.status === 500) {
                    // Token expired or invalid - clear and redirect to login
                    console.log('Token expired during question fetch, redirecting to login');
                    localStorage.removeItem('token');
                    localStorage.removeItem('jwtToken');
                    sessionStorage.removeItem('jwtToken');
                    alert('Your session has expired. Please login again.');
    ...
    async function submitAnswer(answer) {
        ...
        try {
            ...
            const response = await fetch('http://127.0.0.1:13371/game/check_answer', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem("token")}`
            ...
            if (!response.ok) {
                if (response.status === 401 || response.status === 500) {
                    // Token expired or invalid - clear and redirect to login
                    console.log('Token expired during answer check, redirecting to login');
                    localStorage.removeItem('token');
                    localStorage.removeItem('jwtToken');
                    sessionStorage.removeItem('jwtToken');
                    alert('Your session has expired. Please login again.');
    ...
    async function handleAnswerResult(result, buttonElement, questionType) {
        // Backend returns status: "correct", "win", "lose", or "incorrect"
        ...
        if (isCorrect) {
            ...
            setTimeout(() => {
                ...
                setTimeout(async () => {
                    ...
                    // FIXED: Check win condition from backend status, not local HP
            ...
            setTimeout(() => {
                ...
                setTimeout(async () => {
                    ...
                    // FIXED: Check lose condition from backend status, not local HP
    ...
    async function handleGameEnd(isWin, reward) {
        ...
        if (isWin) {
            ...
            await saveProgressToBackend(reward);
        }
    ...
    async function saveProgressToBackend(reward) {
        try {
            ...
            if (incrementResponse.status === 401) {
                // Only 401 means unauthorized/expired token - redirect to login
                console.log('Token expired during win increment, redirecting to login');
                localStorage.removeItem('token');
                localStorage.removeItem('jwtToken');
                sessionStorage.removeItem('jwtToken');
                alert('Your session has expired. Please login again.');
            ...
            if (reward > 0) {
                ...
                if (userResponse.status === 401) {
                    // Only 401 means unauthorized/expired token - redirect to login
                    console.log('Token expired during user update, redirecting to login');
                    localStorage.removeItem('token');
                    localStorage.removeItem('jwtToken');
                    sessionStorage.removeItem('jwtToken');
                    alert('Your session has expired. Please login again.');
                    ...
                    console.error('Failed to update user reward:', errorText);
                    console.error('Payload sent:', {
                        money_increment: reward
                    });
                    // Continue anyway - don't block user from seeing results
    ...
    function getGameDifficulty() {
        ...
        // If no difficulty param, derive from level
        if (!difficulty) {
