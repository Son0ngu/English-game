    ...
    async function saveProgressToBackend(reward) {
        try {
            ...
            if (incrementResponse.status === 401) {
                console.log('Token expired during progress save, redirecting to login');
                localStorage.removeItem('token');
                localStorage.removeItem('jwtToken');
                sessionStorage.removeItem('jwtToken');
                alert('Your session has expired. Please login again.');
            ...
            if (reward > 0) {
                ...
                if (userResponse.status === 401) {
                    console.log('Token expired during reward update, redirecting to login');
                    localStorage.removeItem('token');
                    localStorage.removeItem('jwtToken');
                    sessionStorage.removeItem('jwtToken');
                    alert('Your session has expired. Please login again.');
                ...
                if (userResponse.ok) {
                    ...
                    console.error('Failed to add reward:', errorText);
                    // Continue anyway - don't block user from seeing results
                }
            }
        } catch (error) {
